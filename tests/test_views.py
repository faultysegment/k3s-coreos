...
class TestTUIView(unittest.TestCase):
    ...
    def test_show_header(self, mock_print):
        ...
        self.assertTrue(
            any(
                hasattr(call[0][0], "renderable") or "CoreOS ISO Creator" in str(call)
    ...
    @patch("rich.prompt.Prompt.ask")
    @patch.object(Console, "print")
    def test_configure_ssh_key_with_available_keys(
        self, mock_print, mock_prompt, mock_ssh_info
    ):
        ...
        table_printed = any(
            isinstance(call[0][0], Table) for call in mock_print.call_args_list
        )
    ...
    def test_show_completion_success(self, mock_print):
        ...
        with patch("os.path.exists", return_value=True), patch(
            "src.models.ButaneFileFinder.get_file_info",
            return_value={"size_str": "100.0 MB"},
        ):
            self.view.show_completion(config)

            # Verify completion panel was displayed
            panel_printed = any(
                "Completed" in str(call) or hasattr(call[0][0], "renderable")
    ...
    def test_show_completion_failure(self, mock_print):
        ...
        with patch("os.path.exists", return_value=False):
            ...
            error_printed = any(
                "ISO file was not created" in str(call)
                for call in mock_print.call_args_list
            )
